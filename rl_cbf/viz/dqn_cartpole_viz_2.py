...
if __name__ == '__main__':
    ...
    initial_state = get_initial_states(1)[0]
    env.reset()
    ...
    while not done:
        ...
        value = model.predict_value([state]).item()
        values.append(value)
        actions.append(action)
        next_state, reward, done, _ = env.step(action)
        next_action = model.predict_action([next_state]).item()
        next_value = model.predict_value([next_state]).item()
