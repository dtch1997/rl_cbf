...
class DQNCartPoleEvaluator:
    def __init__(
        ...
        if self.capture_video:
            self.eval_env = gym.wrappers.RecordVideo(
                self.eval_env, f"./videos/{video_path}"
            )
    ...
    def evaluate_grid(
        self, model: QNetwork, n_grid_points: int = 10000
    ) -> pd.DataFrame:
    ...
    def evaluate_barrier(
        ...
        for episode_idx, initial_state in enumerate(initial_states):
            for action in range(self.eval_env.action_space.n):
                ...
                next_state, reward, done, info = self.eval_env.step(action)
                next_value = model.predict_value(next_state)
                next_barrier_value = model.predict_value(
                    next_state, apply_sigmoid=False
                )
    ...
    def calculate_validity(
        self, df: pd.DataFrame, one_minus_alphas: np.ndarray
    ) -> np.ndarray:
        """Determine whether barrier condition is satisfied for each point"""
        df = df[["episode", "barrier_value", "next_barrier_value", "state"]]
        sup_next_barrier_values = (
            df[["episode", "next_barrier_value"]].groupby("episode").max()
        )
        df = df.drop_duplicates("episode")
        df = pd.merge(
            df[["episode", "barrier_value", "state"]],
            sup_next_barrier_values,
            on="episode",
        )
