...
class DQNCartPoleVisualizer:
    ...
    def visualize(self, model: QNetwork, barrier_threshold = 0, state_history: np.ndarray = None) -> plt.Figure:
        ...
        plot_heatmap_theta(fig, ax[0,0], theta_space, theta_dot_space, state_values)    
        ax[0,0].set_title('Value function')
        if state_history is not None:
            ax[0,0].scatter(state_history[:,2], state_history[:,3], c='k', s=1)
        ...
        plot_heatmap_theta(fig, ax[0,1], theta_space, theta_dot_space, activations)
        ax[0,1].set_title('Barrier function')
        ...
        state_values = torch.Tensor(state_values)
        plot_heatmap_x(fig, ax[1,0], x_space, x_dot_space, state_values)
        ax[1,0].set_title('Value function')
        if state_history is not None:
            ax[1,0].scatter(state_history[:,0], state_history[:,1], c='k', s=1)

        # Plot barrier function
        activations = model.predict_value(states, apply_sigmoid=False)
        activations = activations.reshape(100, 100, 1, 1)
        activations = torch.Tensor(activations)
        plot_heatmap_x(fig, ax[1,1], x_space, x_dot_space, activations)
        ax[1,1].set_title('Barrier function')
